...
def crop_and_resize(image_path: str, person_bbox: tuple, crop_target: str, config: dict):
    ...
    try:
        ...
        if crop_target == "fullbody":
            ...
            crop_box = (px1 + int(p_width * 0.15), py1, px2 - int(p_width * 0.15), py1 + int(p_height * 0.3))
            ...
            crop_box = (px1 + int(p_width * 0.25), py1 + int(p_height * 0.5), px2 - int(p_width * 0.25), py1 + int(p_height * 0.8))
        ...
        if x1 >= x2 or y1 >= y2:
            logger.warning(f"Invalid crop box calculated for {crop_target} in {image_path}. Skipping.")
            return None
        ...
        if target_size:
            resized_img = cv2.resize(cropped_img, tuple(target_size), interpolation=cv2.INTER_LANCZOS4)
        else:
