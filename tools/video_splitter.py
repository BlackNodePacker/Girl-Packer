...
def _run_command(command):
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred running Popen command: {' '.join(command)}. Error: {e}", exc_info=True)
        return False, "", str(e)

def get_video_duration(video_path):
    """Gets video duration in seconds using ffprobe."""
    if not os.path.exists(video_path): return 0
    command = [FFPROBE_PATH, "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", str(video_path)]
    try:
        startupinfo = None
        if os.name == 'nt':
            startupinfo = subprocess.STARTUPINFO()
...
def get_ffmpeg_split_commands(video_path, output_folder, clips):
    ...
    for i, clip_info in enumerate(clips):
        start_ms = clip_info['start']; end_ms = clip_info['end']
        start_time = start_ms / 1000.0; end_time = end_ms / 1000.0
        if end_time <= start_time: continue

        output_path = os.path.join(output_folder, f"{base_name}_clip_{i:03d}{extension}")
        command = [FFMPEG_PATH, '-i', str(video_path), '-ss', str(start_time), '-to', str(end_time), '-c', 'copy', '-avoid_negative_ts', '1', str(output_path), '-y']
...
def generate_clip_timestamps(video_path, params):
    ...
    if duration_s == 0: logger.error("Could not determine video duration."); return [], 0
    timestamps, mode, value = [], params['mode'], params['value']
    if mode == "duration":
        for start_s in range(0, int(duration_s), value):
            end_s = min(start_s + value, duration_s); timestamps.append({'start': start_s * 1000, 'end': end_s * 1000})
    elif mode == "number":
        if value == 0: return [], 0
        segment_duration_s = duration_s / value
        for i in range(value):
            start_s, end_s = i * segment_duration_s, (i + 1) * segment_duration_s; timestamps.append({'start': int(start_s * 1000), 'end': int(end_s * 1000)})
...
def split_video(video_path, output_folder, clips, progress_callback=None):
    ...
    video_path_obj = Path(video_path); base_name = video_path_obj.stem; extension = video_path_obj.suffix
