...
def generate_custom_traits_rpy(
    character_name: str, custom_traits_data: list, output_dir: str
) -> str:
    """Generates an RPY file defining custom traits for a character."""
    if not custom_traits_data:
        return None
    safe_char_name = sanitize_filename(character_name).lower().replace("_", "")
    rpy_content = (
        f"# Generated Custom Traits for {character_name} by Girl Packer Tool\n\ninit -1 python:\n"
    )
    trait_definitions = []
...
def generate_event_rpy(event_config: dict, event_script_data: dict, event_folder: str) -> str:
    ...
    for stage_name in stages:
        ...
        if stage_name == "start":
            ...
            rpy_content += (
                f"            # Assumes the primary participant is the first in the list\n"
            )
