...
try:
    ...
    class PlayerWidget(QWidget):
        ...
        def load_video(self, path: str):
            logging.getLogger("PlayerWidget").warning(
                "VLC library (python-vlc) is not installed. Video preview disabled."
            )
...
class EventMakerPanel(QWidget):
    ...
    def _initialize_default_state(self):
        ...
        self.event_type_combo.addItems(
            ["home_visit", "post_exam", "dream", "home_visit_early", "school_day"]
        )
        ...
        try:
            if hasattr(self.project_manager, "get_all_traits"):
                ...
                logger.warning(
                    f"Project Manager object ({type(self.project_manager).__name__}) is missing 'get_all_traits' method."
                )
                all_traits = []
    ...
    def _setup_general_tab(self):
        ...
        self.event_type_combo.setSizePolicy(
        ...
        options_group = self._create_group_box_with_layout(
            "Event Options & Cooldowns", options_layout
        )
    ...
    def _setup_participant_impacts_section(self):
        ...
        self.impact_participant_combo.currentTextChanged.connect(
            self._on_impact_participant_changed
        )
        ...
        self.participant_stats_table = self._create_group_box_with_layout(
            "Stat Impacts (XP)", stats_table_layout
        )
        ...
        self.traits_to_add_list.setSizePolicy(
        ...
        add_trait_btn.clicked.connect(
            lambda: self._add_trait_to_list(self.trait_add_combo, self.traits_to_add_list)
        )
        ...
        self.traits_to_remove_list.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        ...
        remove_trait_btn.clicked.connect(
            lambda: self._add_trait_to_list(self.trait_remove_combo, self.traits_to_remove_list)
        )
    ...
    def _setup_requirements_section(self):
        ...
        stats_table_widget.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.stat_requirements_table = self._create_group_box_with_layout(
            "Stat Requirements", QVBoxLayout()
        )
        ...
        req_traits_table_widget.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self.required_traits_table = self._create_group_box_with_layout(
            "Required Traits", QVBoxLayout()
        )
        ...
        forbid_traits_table_widget.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self.forbidden_traits_table = self._create_group_box_with_layout(
            "Forbidden Traits", QVBoxLayout()
        )
        ...
        action_reqs_table_widget.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self.action_reqs_table = self._create_group_box_with_layout(
            "Action Requirements", QVBoxLayout()
        )
        ...
        accept_influences_table_widget.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self.accept_influences_table = self._create_group_box_with_layout(
            "Accept Influences", QVBoxLayout()
        )
        self.accept_influences_table.findChild(QVBoxLayout).addWidget(
            accept_influences_table_widget
        )
    ...
    def _setup_script_editor_tab(self):
        ...
        stage_group = self._create_group_box_with_layout(
            "Stage Management", stage_management_layout
        )
        main_layout.addWidget(stage_group, 1)

        # Middle: Script Editor (3/5 of the width)
        script_editor_layout = QVBoxLayout()
        script_editor_layout.addWidget(
            QLabel("Ren'Py Script Content (Indentation is handled automatically):")
        )
    ...
    def _setup_rpy_tools_section(self):
        ...
        self.end_early_btn.clicked.connect(
            lambda: self._insert_text_to_editor(
    ...
    def _setup_media_library_tab(self):
        ...
        self.media_list_widget.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        ...
        list_group = self._create_group_box_with_layout(
            "Media Library (Scripting Names)", list_layout
        )
        ...
        if asset_type == "image":
            ...
            try:
                ...
                self.image_preview.setText(
                    f"VIDEO ERROR: Failed to load with VLC ({type(e).__name__})."
                )
        else:
    ...
    def _on_quick_insert(self, asset_type):
        ...
        if asset_info["type"] == "image" and asset_type == "image":
            ...
            QMessageBox.warning(
                self,
                "Warning",
                f"Cannot insert {asset_type} RPY code for a {asset_info['type']} asset.",
            )
        ...
        try:
            ...
            if hasattr(self.project_manager, "get_asset_file_paths"):
                ...
                raise AttributeError(
                    "Project Manager is missing the required media retrieval method: 'get_asset_file_paths'."
                )

            if not file_paths:
                QListWidgetItem(
                    "No media files found in the current project assets.", self.media_list_widget
                )
                return
            ...
            for path in file_paths:
                ...
                self.available_assets[script_name] = {
                    "path": path,
                    "type": asset_type,
                    "rpy_path_hint": os.path.join(
                        os.path.basename(os.path.dirname(path)), file_name
                    ),
                }
            ...
            self.media_list_widget.clear()
            QListWidgetItem(
                f"ERROR: Could not load media files. Check logs: {type(e).__name__}",
                self.media_list_widget,
            )
    ...
    def _add_dialog(self):
        ...
        char_id, ok = QInputDialog.getItem(
            self, "Add Dialog", "Select Character ID:", char_list, 0, False
        )
        if not ok:
            return

        dialog, ok_d = QInputDialog.getText(
            self, "Add Dialog", f"Enter the dialogue for '{char_id}':"
        )
        if not ok_d:
            return
    ...
    def _add_show_image(self, script_name: str = None):
        ...
        if not script_name:
            script_name, ok = QInputDialog.getText(
                self,
                "Add Show Image",
                "Enter Image Script Name (e.g., 'pic1', 'my_custom_image'):",
                QLineEdit.Normal,
                "",
            )
    ...
    def _add_show_video(self, script_name: str = None):
        ...
        if not script_name:
            script_name, ok = QInputDialog.getText(
                self,
                "Add Show Video",
                "Enter Video Script Name (e.g., 'vid1', 'my_custom_video'):",
                QLineEdit.Normal,
                "",
            )
            if not ok or not script_name.strip():
    ...
    def _add_menu_block(self):
        ...
        menu_name, ok = QInputDialog.getText(
            self, "Menu Block", "Enter name for the menu label (optional, e.g., 'choice_1'):"
        )
        if not ok:
            return
    ...
    def _add_participant(self, initial=False):
        ...
        if p_type == "player":
            ...
            if not initial:
                QMessageBox.warning(
                    self, "Invalid ID", "Please enter an ID suffix for this participant type."
                )
            return
        ...
        if pid in current_items:
            if not initial:
                QMessageBox.warning(
                    self, "Duplicate Participant", f"Participant ID '{pid}' already exists."
                )
            return
    ...
    def _save_current_participant_impacts(self):
        ...
        self.participant_impacts_data[pid]["add_traits"] = self._get_list_widget_items(
            self.traits_to_add_list
        )
        self.participant_impacts_data[pid]["remove_traits"] = self._get_list_widget_items(
            self.traits_to_remove_list
        )
    ...
    def _add_trait_to_list(self, combo: QComboBox, target_list: QListWidget):
        ...
        for i in range(target_list.count()):
            if target_list.item(i).text() == trait_tag:
                QMessageBox.warning(
                    self, "Duplicate Trait", f"Trait '{trait_tag}' is already in the list."
                )
                return
    ...
    def _add_stage(self, initial=False, name=None):
        ...
        if initial and self.stages_list.count() > 0:
            return

        if not name:
            new_stage_name = f"stage_{self.stages_list.count() + 1}"
            stage_name_dialog, ok = QInputDialog.getText(
                self,
                "New Stage Name",
                "Enter the Ren'Py label name for the new stage:",
                QLineEdit.Normal,
                new_stage_name,
            )
            if ok and stage_name_dialog.strip():
                ...
                if not initial:
                    QMessageBox.warning(
                        self, "Cancelled", "Stage creation cancelled or name was empty."
                    )
                return
    ...
    def _save_current_stage_script(self, stage_name=None):
        ...
        if self.stages_list.currentRow() < 0:
            return
    ...
    def _on_save_event(self):
        ...
        required_traits_data = self._get_table_data(
            self.required_traits_table.findChild(QTableWidget)
        )
        forbidden_traits_data = self._get_table_data(
            self.forbidden_traits_table.findChild(QTableWidget)
        )
        ...
        accept_influences_dict = {
            item[0]: float(item[1])
            for item in accept_influences_data
            if len(item) == 2 and item[0] and item[1]
        }
    ...
    def _generate_rpy_script_content(self, normalized_event_name):
        ...
        for stage_name in stage_names:
            ...
            if stage_name == stage_names[0] and "girl" in self._get_list_widget_items(
