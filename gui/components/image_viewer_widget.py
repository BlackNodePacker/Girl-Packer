...
class ImageViewerWidget(QWidget):
    ...
    def set_box_visibility(self, detection: dict, visible: bool):
        ...
        if self.selected_box_detection and self.selected_box_detection.get('id') == target_id and not visible:
    ...
    def get_scaled_rect(self, rect: QRect) -> QRect:
        ...
        return QRect(int(rect.x() * scale_x), int(rect.y() * scale_y), 
    ...
    def paintEvent(self, event):
        ...
        for rect, label, color, detection in self.boxes:
            ...
            is_selected = self.selected_box_detection and self.selected_box_detection.get('id') == detection.get('id')
    ...
    def _get_handle_at_point(self, point: QPoint):
        ...
        if self.selected_box_detection and self.selected_box_detection.get('visible', True) and scaled_rect.contains(point): 
            return 'move'
        ...
        if self.current_mode == self.MODE_DRAW:
            ...
            if handle:
                ...
                if self.drag_handle in ['top_left', 'top_right', 'bottom_left', 'bottom_right']:
                    ...
                    if self.drag_handle == 'top_left': self.fixed_point = current_rect.bottomRight()
                    elif self.drag_handle == 'top_right': self.fixed_point = current_rect.bottomLeft()
                    elif self.drag_handle == 'bottom_left': self.fixed_point = current_rect.topRight()
                    elif self.drag_handle == 'bottom_right': self.fixed_point = current_rect.topLeft()
            ...
            if x_max > x_min and y_max > y_min:
                ...
                for i, (rect, label, color, detection) in enumerate(self.boxes):
                    if detection.get('id') == self.selected_box_detection.get('id'):
                        new_qrect = QRect(int(x1_final), int(y1_final), int(x2_final - x1_final), int(y2_final - y1_final))
