...
class ContactSheetPanel(QWidget):
    ...
    def load_frames(self, frame_paths):
        ...
        self.thread.finished.connect(
            lambda: logger.info("Finished loading all frame widgets into contact sheet.")
        )
    ...
    def confirm_selection(self):
        if not self.selected_frames:
            QMessageBox.warning(
                self, "No Frames Selected", "Please select at least one frame to continue."
            )
            return
