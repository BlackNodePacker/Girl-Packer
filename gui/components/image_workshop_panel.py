...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel,
                               ...
                               QRadioButton, QCheckBox, QComboBox, QApplication, QGridLayout,
                               QInputDialog, QFrame, QToolBar)
...
class LayerItemWidget(QWidget):
    ...
    def __init__(self, detection, color, item_ref):
        ...
        confidence_text = f" ({int(detection.get('confidence', 0) * 100)}%)" if 'confidence' in detection and detection.get('confidence', 0) < 1.0 else ""
        ...
        self.checkbox.stateChanged.connect(lambda state: self.visibility_changed.emit(self.detection, state == Qt.CheckState.Checked))
    ...
    def update_text(self, new_bbox):
        ...
        confidence_text = f" ({int(self.detection.get('confidence', 0) * 100)}%)" if self.detection.get('confidence', 0) < 1.0 else ""
...
class ImageWorkshopPanel(QWidget):
    ...
    def __init__(self, main_window, parent=None):
        ...
        self.color_map = {
            "person": QColor(220,220,220,100), "face": QColor(52,152,219,200), 
            "boobs": QColor(230,126,34,200), "pussy": QColor(142,68,173,200), 
            "ass": QColor(243,156,18,200), "bra": QColor(231,76,60,200), 
            "panty": QColor(155,89,182,200), "default": QColor(0,255,255,150)
        }
    ...
    def _setup_ui(self):
        ...
        self.select_mode_action = self._create_action("fa5s.mouse-pointer", "Select/Move Tool", self._on_select_mode_clicked, True)
        self.draw_mode_action = self._create_action("fa5s.edit", "Draw New Box", self._on_draw_mode_clicked, True)
        self.delete_action = self._create_action("fa5s.trash-alt", "Delete Selected Box", self._on_delete_clicked)
    ...
    def _setup_details_panel(self):
        ...
        for name, tag in self.main_window.tag_manager.get_vid_tags().get('corruption_level', {}).items():
    ...
    def _connect_signals(self):
        ...
        self.layers_list_widget.itemClicked.connect(lambda item: self._on_box_clicked(item.data(Qt.UserRole)))
        ...
        if detection_to_delete:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                if id_to_delete:
                    ...
                    self.yolo_results_cache[path] = [d for d in detections if d.get('id') != id_to_delete]
                    
                    # إزالة من قائمة Tags المؤكدة
                    if path in self.frame_tag_buffer:
                        self.frame_tag_buffer[path] = [item for item in self.frame_tag_buffer[path] if item['yolo_detection'].get('id') != id_to_delete]
    ...
    def _on_delete_clicked(self):
        ...
        if detection_id:
            ...
            # إزالة من قائمة Tags المؤكدة
            if path in self.frame_tag_buffer:
                self.frame_tag_buffer[path] = [item for item in self.frame_tag_buffer[path] if item['yolo_detection'].get('id') != detection_id]
            ...
            QMessageBox.warning(self, "Error", "Could not find the selected item in the list to delete.")
    ...
    def _on_new_box_drawn(self, rect: QRect):
        ...
        label, ok = QInputDialog.getItem(self, "New Box", "Select a label for the new box:", labels, 0, False)
    ...
    def _display_current_frame(self):
        ...
        self.frame_counter_label.setText(f"Frame: {self.current_frame_index + 1}/{len(self.frame_paths)}")
        ...
        for det in detections:
            ...
            is_confirmed = next((True for item in self.frame_tag_buffer.get(self.current_frame_path, []) if item['yolo_detection'].get('id') == det['id']), False)
    ...
    def _on_box_clicked(self, detection: dict):
        ...
        try:
            ...
            if cropped_image.size > 0 and hasattr(self.main_window.pipeline, 'classify_asset') and self.main_window.pipeline.classify_asset:
        ...
        self._on_yolo_override_changed(current_yolo_label, cnn_suggestion_for_base_type=cnn_suggestion)
    ...
    def _load_existing_tags(self, detection_data: dict):
        ...
        tagged_item = next((
            item for item in self.frame_tag_buffer.get(self.current_frame_path, []) 
            if item['yolo_detection'].get('id') == detection_id
        ), None)

        if tagged_item:
            logger.info(f"Loading existing tags for ID {detection_id}: {tagged_item.get('final_name')}")

            # [FIX] يجب تحديث الـ YOLO Override Combo أولاً إذا كان التاغ موجوداً
            yolo_override_label = tagged_item.get('yolo_label_override', detection_data.get('label'))
            ...
            if base_idx >= 0:
                ...
                if self.current_source_key == 'clothing':
                    source_map = self.main_window.tag_manager.get_clothing_map().get(tagged_item.get('base_type'), {})
                    # البحث عن مفتاح القاموس باستخدام القيمة (final_class_tag)
                    final_class_text_lookup = next((k for k, v in source_map.items() if v == final_class_tag), final_class_tag)
                else:
                ...
                state_match = next((tag for tag in self.corruption_radios.keys() if tag and f"-{tag}" in final_name), None)
    ...
    def _on_yolo_override_changed(self, new_label, cnn_suggestion_for_base_type=None):
        ...
        elif new_label.startswith('portrait') or new_label in ['face', 'boobs', 'pussy', 'ass', 'legs']:
    ...
    def _confirm_current_item(self):
        ...
        if self.current_source_key == 'clothing':
            ...
            yolo_label_for_sizing = self.current_selected_detection.get('label', 'default_asset').lower()
        ...
        if not final_class_tag:
            QMessageBox.critical(self, "Tag Error", f"Could not determine final tag for {base_type_text} / {final_class_text}")
            return


        final_tag_parts = [final_class_tag]
        
        corruption_tag = next((tag for tag, radio in self.corruption_radios.items() if radio.isChecked()), "")
        if corruption_tag:
        ...
        if not cover_type: cover_type = 'misc'
        if not body_part_cover: body_part_cover = 'default_cover'
        ...
        logger.info(f"Confirmed Item. Tag: {new_tagged_item['final_name']}, Category: {asset_category}")
        ...
        self.frame_tag_buffer[current_path] = [
            item for item in self.frame_tag_buffer[current_path] 
            if item['yolo_detection'].get('id') != detection_id
        ]
    ...
    def _check_and_finish_workshop(self):
        ...
        for path in self.frame_paths:
            ...
            unconfirmed_count = len([det for det in yolo_dets if det.get('id') not in confirmed_ids])
        ...
        if missing_tags_count > 0:
            reply = QMessageBox.question(self, "Unconfirmed Items", 
