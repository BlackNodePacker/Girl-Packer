...
class ShootMakerPanel(QWidget):
    ...
    def _create_right_panel(self):
        ...
        self.shoot_subtype_combo.addItems(
            ["--- Select ---"] + list(self.shoots_tags.get("shoot_subtypes", {}).keys())
        )
        ...
        self.participants_combo.addItems(
            ["--- (Auto) ---"] + list(self.shoots_tags.get("participant_tags", {}).keys())
        )
        ...
        self.location_combo.addItems(
            ["--- (Auto) ---"] + list(self.shoots_tags.get("location_tags", {}).keys())
        )
        ...
        self.theme_combo.addItems(
            ["--- Optional ---"] + list(self.shoots_tags.get("theme_tags", {}).keys())
        )
    ...
    def _connect_signals(self):
        ...
        self.available_list.currentRowChanged.connect(
            lambda: self._preview_media(self.available_list)
        )
    ...
    def _on_shared_toggled(self, checked):
        ...
        if checked:
            char_id = (
                sanitize_filename(self.main_window.project.character_name).lower().replace(" ", "_")
            )
    ...
    def _setup_step2_main_editor(self):
        ...
        main_layout = QVBoxLayout(self.step2_widget)
        title = QLabel("Step 2: Build & Configure Your Shoot")
        title.setObjectName("TitleLabel")
        main_layout.addWidget(title)
        top_layout = QHBoxLayout()
        top_layout.addLayout(self._create_left_panel(), 2)
        top_layout.addLayout(self._create_center_panel(), 4)
        top_layout.addLayout(self._create_right_panel(), 3)
        main_layout.addLayout(top_layout)
        bottom_bar = QHBoxLayout()
        back_button = QPushButton("⬅️ Back to Type Selection")
        self.save_button = QPushButton("✔️ Save Shoot to Project")
        self.save_button.setObjectName("ConfirmButton")
    ...
    def _create_left_panel(self):
        ...
        shoot_group = QGroupBox("Items in Shoot (Sequence)")
        shoot_layout = QVBoxLayout(shoot_group)
        list_hbox = QHBoxLayout()
        self.shoot_list = QListWidget()
        list_hbox.addWidget(self.shoot_list)
        arrows_vbox = QVBoxLayout()
        self.move_up_button = QPushButton("⬆️")
        self.move_down_button = QPushButton("⬇️")
        arrows_vbox.addWidget(self.move_up_button)
        arrows_vbox.addWidget(self.move_down_button)
        arrows_vbox.addStretch()
        list_hbox.addLayout(arrows_vbox)
        shoot_layout.addLayout(list_hbox)
        self.remove_button = QPushButton("⬆️ Remove from Shoot ⬆️")
        shoot_layout.addWidget(self.remove_button)
    ...
    def _create_center_panel(self):
        ...
        tagging_group = QGroupBox("Selected Item Tagging")
        self.item_tagging_box = tagging_group
        self.item_tagging_box.setEnabled(False)
        tag_layout = QGridLayout(tagging_group)
        self.main_tag_combo = self._create_combo(
            "Main Tag:", self.shoots_tags.get("item_main_tags", {}).keys(), tag_layout, 0
        )
        self.action_group_combo = self._create_combo(
            "Action Group:", self.shoots_tags.get("item_action_tags", {}).keys(), tag_layout, 1
        )
        self.specific_action_combo = self._create_combo("Specific Action:", [], tag_layout, 2)
        self.apply_tags_button = QPushButton("Apply Tags to Selected Item")
        tag_layout.addWidget(self.apply_tags_button, 4, 0, 1, 2)
        layout.addWidget(tagging_group)
        return layout
    ...
    def _preview_media(self, list_widget: QListWidget):
        ...
        if self.current_shoot_type == "Photo Shoot":
            ...
            self.video_preview_player.load_video(path)
        if list_widget is self.shoot_list:
            self.item_tagging_box.setEnabled(True)
            tags = self.shoot_data_buffer.get(path, {}).get("tags", {})
            self.main_tag_combo.setCurrentText(tags.get("main_tag_text", "---"))
            self.action_group_combo.setCurrentText(tags.get("action_group", "---"))
            self._on_action_group_changed(self.action_group_combo.currentText())
            self.specific_action_combo.setCurrentText(tags.get("specific_action", "---"))
        else:
            self.item_tagging_box.setEnabled(False)

    def activate(self):
        self.step_stack.setCurrentWidget(self.step1_widget)
        can_video_shoot = self.main_window.project.source_type == "video" and bool(
            self.main_window.project.export_data.get("all_created_clips")
        )
        self.video_shoot_radio.setEnabled(can_video_shoot)
        if not can_video_shoot:
            self.photo_shoot_radio.setChecked(True)
            self.video_shoot_radio.setToolTip("Create clips in 'Vid Maker' first to enable this.")

    def display_sources(self, sources):
        self.available_list.clear()
        self.shoot_list.clear()
        self.shoot_data_buffer.clear()
    ...
    def apply_ai_suggestions(self, suggestions):
        if suggestions.get("participants"):
            self.participants_combo.setCurrentText(suggestions["participants"])
        if suggestions.get("location_tag"):
            loc_tag = suggestions["location_tag"]
            for name, tag in self.shoots_tags.get("location_tags", {}).items():
                if tag == loc_tag:
                    self.location_combo.setCurrentText(name)
                    break

    def _on_start_shoot(self):
        if self.photo_shoot_radio.isChecked():
            self.current_shoot_type = "Photo Shoot"
            self.preview_stack.setCurrentWidget(self.image_preview_label)
        else:
            self.current_shoot_type = "Video Shoot"
            self.preview_stack.setCurrentWidget(self.video_preview_player)
    ...
    def _move_item(self, direction):
        ...
        if row > -1 and 0 <= row + direction < self.shoot_list.count():
            item = self.shoot_list.takeItem(row)
            self.shoot_list.insertItem(row + direction, item)
            self.shoot_list.setCurrentRow(row + direction)

    def _on_action_group_changed(self, group_name):
        self.specific_action_combo.clear()
        self.specific_action_combo.addItems(["---"])
        if not group_name or group_name == "---":
            return
        actions_for_group = self.shoots_tags.get("item_action_tags", {}).get(group_name, {})
        if actions_for_group:
            self.specific_action_combo.addItems(list(actions_for_group.keys()))
    ...
    def _apply_item_tags(self):
        ...
        tags_to_apply = {
            ...
            "specific_action": self.specific_action_combo.currentText(),
        }
    ...
    def _on_save_shoot(self):
        ...
        if (
            not shoot_name
            or not display_name
            or self.shoot_subtype_combo.currentIndex() <= 0
            or self.shoot_list.count() == 0
        ):
            return QMessageBox.warning(
                self,
                "Incomplete Info",
                "Shoot Name, Display Name, Subtype, and at least one item are required.",
            )
        shoot_key = sanitize_filename(shoot_name)
        all_tags = set()
        shoot_subtype_val = self.shoots_tags["shoot_subtypes"][
            self.shoot_subtype_combo.currentText()
        ]
        all_tags.add(shoot_subtype_val)
        if self.participants_combo.currentIndex() > 0:
            all_tags.add(
                self.shoots_tags["participant_tags"][self.participants_combo.currentText()]
            )
        if self.location_combo.currentIndex() > 0:
            all_tags.add(self.shoots_tags["location_tags"][self.location_combo.currentText()])
        if self.theme_combo.currentIndex() > 0:
            all_tags.add(self.shoots_tags["theme_tags"][self.theme_combo.currentText()])
        config = {
            "name": shoot_key,
            "display_name": display_name,
            "shoot_subtype": shoot_subtype_val,
            "tags": sorted(list(filter(None, all_tags))),
            "cost": self.cost_spinbox.value(),
            "description": self.description_input.toPlainText().strip(),
            "is_shared": self.is_shared_checkbox.isChecked(),
        }
        if config["is_shared"]:
            ids_text = self.participant_ids_input.text().strip()
            if ids_text:
                participant_ids = [pid.strip() for pid in ids_text.split(",") if pid.strip()]
        ...
        for i in range(self.shoot_list.count()):
            ...
            if (
                ...
                or data["tags"].get("main_tag_text", "---") == "---"
            ):
                return QMessageBox.warning(
                    self,
                    "Untagged Item",
                    f"Item '{os.path.basename(path)}' has not been tagged with a Main Tag.",
                )
            tags = data["tags"]
            main_tag_val = self.shoots_tags["item_main_tags"].get(tags.get("main_tag_text"), "")
            action_group_dict = self.shoots_tags["item_action_tags"].get(
                tags.get("action_group"), {}
            )
            ...
            if main_tag_val:
                main_parts.append(main_tag_val)
            ...
            media_items.append(
                {"final_name": sanitize_filename(final_filename), "source_path": path}
            )
        final_data = {
            "shoot_type": self.current_shoot_type,
            "shoot_key": shoot_key,
            "config": config,
