...
class AIAnalysisWorker(QObject):
    ...
    def run(self):
        ...
        dominant_location_tag = max(location_counts, key=location_counts.get) if location_counts else None
        
        suggestions = {'participants': participant_suggestion, 'location_tag': dominant_location_tag}
...
class ShootMakerWorkflow(QObject):
    ...
    def save_shoot_to_project(self, shoot_data: dict):
        try:
            shoot_type = shoot_data.get("shoot_type"); shoot_key = shoot_data.get("shoot_key")
            config = shoot_data.get("config"); media_items = shoot_data.get("media_items")
            if not all([shoot_type, shoot_key, config, media_items]):
                logger.error("Attempted to save shoot with incomplete data."); return

            export_key, media_key = ("", "")
            if shoot_type == "Photo Shoot":
                export_key = "_shared_photoshoots" if config.get("is_shared") else "photoshoots"; media_key = "images"
            else: # Video Shoot
                export_key = "_shared_videoshoots" if config.get("is_shared") else "videoshoots"; media_key = "videos"
                
            if export_key not in self.project.export_data: self.project.export_data[export_key] = {}
                
            self.project.export_data[export_key][shoot_key] = {'config': config, media_key: media_items}
