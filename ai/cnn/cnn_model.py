...
def train_pytorch_model(data_dir: str, model_save_path: str, map_save_path: str, num_epochs=25, batch_size=4):
    """
    ...
    try:
        image_datasets = {
            'train': datasets.ImageFolder(train_dir, data_transforms['train'], is_valid_file=is_valid_image_file),
            'val': datasets.ImageFolder(val_dir, data_transforms['val'], is_valid_file=is_valid_image_file)
        }
        dataloaders = {
        ...
        logger.info(f"Datasets created. Training images: {dataset_sizes['train']}, Validation images: {dataset_sizes['val']}")
    except Exception as e:
        logger.error(f"Failed to create datasets. Check data paths and folder structure. Error: {e}")
        return
